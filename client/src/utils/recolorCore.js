/**
 * recolorCore — the PURE, DOM-free sprite-recolor pixel algorithm.
 *
 * This is the single source of truth for the recolor math. It is imported by:
 *   - recolorWorker.js          (browser Web Worker — off-main-thread recolor)
 *   - SpriteRecolorizer.js       (browser main-thread fallback)
 *   - scripts/bakeSprites.mjs    (Node build-time bake)
 *
 * Because all three share THIS function, baked PNGs are pixel-identical to the
 * runtime canvas recolor — that parity is the whole point of extracting it.
 *
 * Nothing in here may touch `window`, `document`, `Image`, `canvas`, or any
 * other browser global: it operates only on a raw RGBA byte buffer
 * (`imageData.data`, a Uint8ClampedArray in the browser or a Node Buffer from
 * pngjs — both index-addressable the same way).
 */

// ============================================
// HSL color range definitions
// ============================================

// BLUE mawashi/headband (the sprite base color)
...
// RED mawashi/headband (legacy player-2 default range; kept for API parity)
...
/** The base color of the sprite assets. If a target matches this, the mawashi
 *  recolor is skipped to avoid HSL round-trip artifacts. */
export const SPRITE_BASE_COLOR = "#4169E1";

// ============================================
// Special (per-pixel pattern) color identifiers
// ============================================
...
export function isColorInHslRange(h, s, l, colorRange) {
 ...
 * x/y are RELATIVE to the centroid of all matching pixels so the pattern stays
 * locked to the body across animation frames.
 */
export function getSpecialPixelColor(specialMode, x, y) {
  switch (specialMode) {
    case "rainbow": {
      const CYCLE = 50;
    ...
    case "galaxy": {
      ...
      if (val < 3) return { h: 200, s: 10, l: 98 };
      if (val < 5) return { h: 45, s: 15, l: 95 };
 ...
 *   Pass 1 – find the centroid of all matching pixels (stable across frames).
 *   Pass 2 – recolor, using coordinates RELATIVE to the centroid.
 *
 * Tints (hit/charge/blubber/armor) are applied AFTER the base recolor so the
 * fighter keeps their colors while the tint reads as a brief overlay.
 *
 * @param {{data: Uint8ClampedArray|Buffer}} imageData
 */
export function processImageData(
  ...
  const bodyTintRgb = bodyColorRange
    ? hslToRgb(bodyTargetHue, bodyTargetSat, bodyTargetLight)
    : null;
  ...
  if (bodyColorRange) {
    ...
    // Eye protection: small enclosed white regions (sclera) are excluded from
    // the body white-tint and instead whitened.
    const MIN_SCLERA = 20;
    ...
    if (isColorInHslRange(pixelHsl.h, pixelHsl.s, pixelHsl.l, sourceColorRange)) {
      ...
      if (!skipMawashiRecolor) {
        ...
        if (specialMode) {
          ...
          const sc = getSpecialPixelColor(specialMode, relX, relY);
          hue = sc.h;
          sat = sc.s;
          light = sc.l;
        }
      ...
      if (edgeNeighbors >= 2) {
        if (hitTintRed) {
          ...
          data[i] = Math.round((1 - ARMOR_PINK_BLEND) * r + ARMOR_PINK_BLEND * ARMOR_PINK_RGB.r);
          data[i + 1] = Math.round((1 - ARMOR_PINK_BLEND) * g + ARMOR_PINK_BLEND * ARMOR_PINK_RGB.g);
          data[i + 2] = Math.round((1 - ARMOR_PINK_BLEND) * b + ARMOR_PINK_BLEND * ARMOR_PINK_RGB.b);
        }
        continue;
      }
