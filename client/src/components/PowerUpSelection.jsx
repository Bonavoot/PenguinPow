...
const PowerUpIcon = styled.div`
  ...
  background: ${props => {
    switch(props.$type) {
      case 'speed': return 'linear-gradient(135deg, #00d2ff, #3a7bd5)';
      case 'power': return 'linear-gradient(135deg, #ff6b6b, #ee5a52)';
      case 'snowball': return 'linear-gradient(135deg, #74b9ff, #0984e3)';
...
const PowerUpSelection = ({ roomId, playerId, onSelectionComplete, onSelectionStateChange }) => {
  const { socket } = useContext(SocketContext);
  const [selectedPowerUp, setSelectedPowerUp] = useState(null);
  const [selectionStatus, setSelectionStatus] = useState({ selectedCount: 0, totalPlayers: 2 });
  ...
  const powerUpInfo = {
    ...
    power: {
      name: 'Power', 
      description: 'Increased knockback damage',
      icon: '💥'
    },
    snowball: {
      name: 'Snowball',
      description: 'Throw projectiles with F key',
      icon: '❄️'
    }
  };
    ...
    <PowerUpSelectionOverlay>
      <PowerUpContainer>
        ...
        <StatusContainer>
          ...
          <StatusText>
            Players Ready: {selectionStatus.selectedCount}/{selectionStatus.totalPlayers}
          </StatusText>
          <TimerText $urgent={timeLeft <= 5}>
            {timeLeft > 0 ? `${timeLeft} seconds remaining` : 'Auto-selecting Speed...'}
          </TimerText>
        </StatusContainer>
      </PowerUpContainer>
    </PowerUpSelectionOverlay>
