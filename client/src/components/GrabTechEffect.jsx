...
// Dramatic shockwave burst animation — frost/blue color scheme
...
const EffectContainer = styled.div`
  ...
  filter:
    saturate(1.12)
    brightness(1.08)
    drop-shadow(0 0 4px rgba(100, 180, 255, 0.25));
`;

/* Slightly larger radius so tech reads clearer in motion */
...
const ShockwaveRing = styled.div`
  ...
  border-radius: 50%;
  border: clamp(5px, 0.4vw, 10px) solid rgba(100, 200, 255, 0.98);
  box-shadow:
    0 0 16px rgba(80, 180, 255, 0.65),
    0 0 30px rgba(60, 160, 255, 0.35),
    0 0 44px rgba(40, 140, 230, 0.2);
  transform: translate(-50%, -50%) scale(0);
...
const InnerFlash = styled.div`
  ...
  background: radial-gradient(
    ...
    rgba(185, 220, 255, 0.95) 24%,
    rgba(80, 180, 255, 0.92) 54%,
    transparent 100%
  );
  transform: translate(-50%, -50%) scale(0);
...
const Spark = styled.div`
  ...
  border-radius: 50%;
  box-shadow:
    0 0 clamp(${props => props.$size * 2.3}px, ${props => (props.$size * 0.18).toFixed(2)}vw, ${props => props.$size * 4.6}px) rgba(80, 180, 255, 0.92),
    0 0 clamp(${props => props.$size * 3.1}px, ${props => (props.$size * 0.24).toFixed(2)}vw, ${props => props.$size * 6.2}px) rgba(60, 160, 230, 0.35);
  opacity: 0;
...
const GrabTechEffect = ({ position }) => {
  ...
  const EFFECT_DURATION = 1600; // Match grab break timing

  // Generate spark particles — frost-themed
  ...
  }, [position?.techId, position?.x, position?.y]);
      ...
      {activeEffects.map((effect) => {
        return (
          <div key={effect.id}>
            ...
            {/* Sumo-themed tech announcement banner — centered (no side) */}
