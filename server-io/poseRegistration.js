 ...
 *
 * Converts one authoritative gameplay root into a stable visual placement.
 * NEVER writes back into gameplay / physics / collision.
 *
 * Feature flag: FIGHTER_POSE_GEOMETRY_V2 — manually approved; default ON.
 * Rollback: FIGHTER_POSE_GEOMETRY_V2=0
 *
 * CJS twin for Node tests. Vite/browser uses the ESM copy at
 * client/src/poseGeometry/poseRegistration.js — keep registrations in sync.
 *
 * See CHARACTER_POSE_GEOMETRY_PHASE.md
 */

/**
 * Shared flag parse (Node tests + docs). Client mirrors this via import.meta.env.
 *   unset / ""  → true
 *   1 / true    → true
 *   0 / false   → false
 */
function isPoseGeometryV2Enabled(envValue) {
  const env =
    envValue !== undefined
      ? envValue
      : typeof process !== "undefined" && process.env
        ? process.env.FIGHTER_POSE_GEOMETRY_V2
        : undefined;
...
const MIRROR_RULE = Object.freeze({
 ...
 */
const POSE_REGISTRY = Object.freeze({
...
const ASSET_TO_POSE = Object.freeze(
  Object.fromEntries(
 ...
 */
function poseKeyFromPresentationState(presentationState) {
...
function getPoseRegistration(poseKey) {
  return POSE_REGISTRY[poseKey] || DEFAULT_REG;
}

function listPoseKeys() {
 ...
 */
function resolvePoseRender(opts = {}) {
...
function measurePoseTransitionPop(fromKey, toKey, facing = -1) {
...
function createPoseTransitionDiagStore(max = 32) {
  const items = [];
  return {
    note(entry) {
