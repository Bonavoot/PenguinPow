...
| Aerial landing resolve (Phase A/A.1/A.2) | Server | `server-io/landingResolution.js`, `landingFlags.js`, `ropeJumpStart.js`, `pushboxGeometry.js` (rope jump V2 only) |
...
**Phase A/A.1/A.2 (flagged):** When `ROPE_JUMP_LANDING_V2` is on, side intent locks once early; endpoint refines on that side; commit uses continuous `recommendedCommitT`; remaining arc uses Hermite / brake (A.2). Flag off = legacy fixed target + post-land 18px/tick. See `AERIAL_LANDING_PHASE_A2.md`. Landing diagnostics are debug-net only — not on the production delta wire.
